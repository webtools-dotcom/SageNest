...
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/fiber-pregnancy-constipation-not-helping.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy constipation-and-fiber explainer while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-08 (Added new blog post: gestational-diabetes-glucose-test-failed-causes)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `gestational-diabetes-glucose-test-failed-causes`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata field `updatedAt` used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/gestational-diabetes-glucose-test-failed-causes.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested gestational-diabetes glucose-test explainer article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-08 (Added new blog post: vitamin-b12-absorption-pregnancy-risk-factors)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `vitamin-b12-absorption-pregnancy-risk-factors`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/vitamin-b12-absorption-pregnancy-risk-factors.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested vitamin B12 absorption risk-factors article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-08 (Added new blog post: calcium-pregnancy-prenatal-vitamin-gap)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `calcium-pregnancy-prenatal-vitamin-gap`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/calcium-pregnancy-prenatal-vitamin-gap.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested calcium-in-prenatal gap article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-07 (Added new blog post: pregnancy-weight-gain-overweight-obese-bmi-guidelines)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `pregnancy-weight-gain-overweight-obese-bmi-guidelines`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/pregnancy-weight-gain-overweight-obese-bmi-guidelines.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy BMI-specific weight-gain guidance article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-06 (Added new blog post: listeria-pregnancy-deli-meat-foods-to-avoid)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `listeria-pregnancy-deli-meat-foods-to-avoid`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/listeria-pregnancy-deli-meat-foods-to-avoid.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested Listeria-and-pregnancy article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-06 (Added new blog post: pregnancy-constipation-fiber-iron-supplements)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `pregnancy-constipation-fiber-iron-supplements`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/pregnancy-constipation-fiber-iron-supplements.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy constipation article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-05 (Added new blog post: protein-pregnancy-first-trimester-nausea-crackers)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `protein-pregnancy-first-trimester-nausea-crackers`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/protein-pregnancy-first-trimester-nausea-crackers.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested first-trimester protein-intake article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-05 (Added new blog post: vitamin-d-deficiency-pregnancy-prenatal-test)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `vitamin-d-deficiency-pregnancy-prenatal-test`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/vitamin-d-deficiency-pregnancy-prenatal-test.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested prenatal low-vitamin-D article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-04-01 (Added new blog post: sciatica-shooting-leg-pain-pregnancy)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `sciatica-shooting-leg-pain-pregnancy`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/sciatica-shooting-leg-pain-pregnancy.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy sciatica article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.
...
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/bleeding-gums-pregnancy-gingivitis.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy gingivitis article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-03-31 (Added new blog post: restless-legs-pregnancy-third-trimester)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `restless-legs-pregnancy-third-trimester`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/restless-legs-pregnancy-third-trimester.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy restless legs third-trimester article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.
...
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/itchy-palms-feet-pregnancy-cholestasis.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested cholestasis-warning pregnancy article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-03-29 (Added new blog post: pregnancy-rhinitis-stuffy-nose-not-a-cold)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `pregnancy-rhinitis-stuffy-nose-not-a-cold`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/pregnancy-rhinitis-stuffy-nose-not-a-cold.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pregnancy rhinitis article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.
...
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/mid-cycle-spotting-ovulation-what-it-means.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested ovulation spotting article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-03-27 (Added new blog post: folic-acid-vs-methylfolate-pregnancy-mthfr)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `folic-acid-vs-methylfolate-pregnancy-mthfr`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/folic-acid-vs-methylfolate-pregnancy-mthfr.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested folic-acid-vs-methylfolate article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.
...
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/vegetarian-pregnancy-dha-nutrient-gaps.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested vegetarian-pregnancy DHA nutrient-gaps article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.

## 2026-03-25 (Added new blog post: pregnancy-ice-craving-pagophagia-iron-deficiency)

- Added a new top-of-list blog post object in `src/data/blogPosts.ts` for slug `pregnancy-ice-craving-pagophagia-iron-deficiency`, including the provided title, description, reading time, image prompt, FAQ entries, and full long-form content, plus required schema metadata fields (`lastReviewed` and `updatedAt`) used by the existing blog pipeline.
- Regenerated the derived publish/index artifacts so the new article is live and consistent across static outputs: `public/blog-static/pregnancy-ice-craving-pagophagia-iron-deficiency.html`, `public/_redirects`, and `public/sitemap.xml`.
- Why: publish the requested pagophagia-and-iron-deficiency pregnancy article while keeping SageNest's canonical blog source and generated blog/redirect/sitemap artifacts synchronized.
