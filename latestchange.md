...
## 2026-02-16 (deployment parse fix for weight gain page)

- Revalidated and cleaned `src/pages/PregnancyWeightGainCalculator.tsx` structure so the page compiles consistently in CI/Cloudflare builds.
- Tightened numeric input guards in the page calculation memo (`isValidPositiveNumber` + finite week parsing) so invalid/empty values safely return no result state instead of propagating `NaN`.
- Re-ran the full production build (`npm run build`) to confirm TypeScript and Vite pass end-to-end.
