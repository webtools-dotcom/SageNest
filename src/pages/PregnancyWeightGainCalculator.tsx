...
const isValidPositiveNumber = (value: number) => Number.isFinite(value) && value > 0;
...
export const PregnancyWeightGainCalculatorPage = () => {
  ...
  const result = useMemo(() => {
    ...
    const parsedWeek = Number(gestationalWeek);

    if (!isValidPositiveNumber(preWeight) || !isValidPositiveNumber(currentWeight) || !isValidPositiveNumber(heightMeters) || !Number.isFinite(parsedWeek)) {
      return null;
    }

    const week = clampToWeekRange(parsedWeek);
