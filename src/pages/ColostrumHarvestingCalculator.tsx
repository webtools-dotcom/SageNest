...
type TimelineStatus = 'past' | 'active' | 'future';
...
const getTimelineStatus = (milestoneDate: Date, today: Date): TimelineStatus => {
  if (today >= milestoneDate) return 'past';
  return 'future';
};
...
const PregnancyTimeline = ({
  ...
  const milestones: TimelineMilestone[] = [
    ...
    },
    {
      ...
      sublabel: '34 weeks — midwife advice only',
      date: earliestStart,
      status: getTimelineStatus(earliestStart, today),
    },
    {
      label: 'Recommended start',
      sublabel: '36 weeks — NHS guidance',
      date: recommendedStart,
      status: today >= recommendedStart ? 'past' : 'future',
    },
    {
      label: 'Due date',
      sublabel: 'Stop collecting when labour begins',
      date: dueDate,
      status: getTimelineStatus(dueDate, today),
    },
  ];

  // Calculate the total span in days for proportional positioning
  const totalDays = Math.max(
    1,
    Math.floor((dueDate.getTime() - today.getTime()) / 86400000),
  );

  const getPercent = (date: Date): number => {
    const days = Math.floor((date.getTime() - today.getTime()) / 86400000);
    return Math.min(100, Math.max(0, (days / totalDays) * 100));
  };

  // The "harvest window" fill: from recommendedStart to dueDate
  const windowStartPct = getPercent(
    today >= recommendedStart ? today : recommendedStart,
  );
  ...
  return (
    ...
    >
      ...
      {/* Track */}
      <div style={{ position: 'relative', margin: '0 auto', maxWidth: '520px' }}>
        {/* Base track line */}
        <div
          style={{
            position: 'relative',
        ...
        >
          ...
          {/* Milestone dots */}
          {milestones.map((m, i) => {
            const pct = i === 0 ? 0 : i === milestones.length - 1 ? 100 : getPercent(m.date);
            const isToday = i === 0;
            ...
            return (
              ...
              >
                ...
                <div
                  style={{
                    width: isRecommended ? '16px' : '12px',
                    height: isRecommended ? '16px' : '12px',
                    borderRadius: '50%',
                    background: isToday
                      ? 'var(--charcoal)'
                      : isPast
                  ...
                  aria-hidden="true"
                />
                {/* Label above for first two, below for last two */}
                <div
                  style={{
                    position: 'absolute',
                    [i < 2 ? 'bottom' : 'top']: '18px',
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
          <span style={{ display: 'inline-block', width: '20px', height: '4px', background: 'var(--sage-light)', borderRadius: '2px' }} />
          Harvest window
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
          <span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', background: 'var(--sage-primary)' }} />
          Key milestone
        </span>
      </div>
    </div>
...
const StatusBadge = ({
  ...
  // Colour scheme per state using existing tokens
  const scheme = isPastDue
    ...
    ? {
        ...
        message: 'You have reached 36 weeks. Begin colostrum harvesting after discussing with your midwife.',
      }
    : {
        bg: '#fffbf0',
        border: '#f0d080',
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* Big countdown number for "not yet" state */}
        {notYet && (
          <p
          ...
          >
            {daysUntilStart} <span style={{ fontSize: '1rem', fontWeight: 400 }}>days to go</span>
          </p>
        )}
    ...
    <main id="main-content" className="container">
      ...
      <section className="hero-section">
        ...
        <p className="hero-description">
          This colostrum harvesting calculator gives your recommended antenatal colostrum harvesting
          start date from your due date. Use it to plan when to start colostrum harvesting at 36 weeks.
        </p>
      </section>

      {/* ── Enhancement 1: Warning box ABOVE the calculator card ── */}
      <section
        style={{
      ...
      >
        <p style={{ marginBottom: 0, color: '#5a4200' }}>
          ...
          36 weeks without explicit midwife or consultant advice, as nipple stimulation can trigger
          uterine contractions. Always discuss with your midwife before starting.
        </p>
      </section>
      ...
      >
        ...
        {/* ── Enhancement 5: Skeleton placeholder before submission ── */}
        {!hasSubmitted && <ResultsSkeleton />}
        ...
        {hasSubmitted && result && dueDateValue ? (
          <>
            ...
            {/* ── Enhancement 4: Colour-coded status badge ── */}
            <StatusBadge
            ...
            {/* ── Enhancement 1: Visual pregnancy timeline ── */}
            <PregnancyTimeline
